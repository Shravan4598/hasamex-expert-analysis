...
class TranscriptLoader:
    ...
    def __init__(
        ...
        Args:
            supported_extensions:
                Optional set of file extensions accepted by the loader.
    ...
    def load_file(
        ...
        Raises:
            SensorException:
                If the path is invalid, unsupported, unreadable,
        ...
        try:
            ...
            if not content.strip():
                raise ValueError(
                    f"Transcript file is empty: {path}"
                )
            ...
            logger.exception(
                "Failed to load transcript file: %s",
                file_path,
            )
    ...
    def load_directory(
        ...
        Raises:
            SensorException:
                If the directory does not exist, is not a directory,
                or cannot be read.
        """
        try:
    ...
    def validate_file(
        ...
        Returns:
            True if the path exists, is a regular file,
    ...
    def _validate_path(
        ...
        if not path.is_file():
            raise IsADirectoryError(
                f"Transcript path is not a file: {path}"
            )
        ...
        if path.suffix.lower() not in self.supported_extensions:
            ...
            raise ValueError(
                f"Unsupported transcript format "
                f"'{path.suffix}'. "
...
def sys_module():
    """
    Return the sys module for compatibility with SensorException.

    The project's SensorException expects the active sys module
    so that traceback information can be extracted correctly.
    """
    import sys
